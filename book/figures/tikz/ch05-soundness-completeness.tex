...
\node[title] at (-4.0,2.8) {Sound nhưng Incomplete};
...
\node[label] at (-4.0,-0.1) {$A \subseteq E$\\Không có false positive\\Bỏ sót một số entailment};
...
\node[label] at (0,-0.1) {$A \not\subseteq E$\\Có false positive\\(kết quả sai)};
...
\node[label] at (4.0,-0.1) {$A = E$\\Chính xác tập hệ quả\\Không thừa, không thiếu};

% Bottom note
\node[font=\scriptsize,align=center,text width=10cm] at (0,-1.2)
  {$E$ = tập hệ quả ngữ nghĩa (entailed conclusions) \quad
   $A$ = tập kết quả thuật toán trả về};
