...
% ====== Decagon C_10 (left) ======
\node[ttl] at (0, 2.5) {Thập giác $C_{10}$};
\foreach \i in {0,...,9} {
...
\node[label, below=0.1cm] at (0, -2.1) {10 nút, mỗi nút bậc 2};

% ====== Two pentagons 2 x C_5 (right) ======
\node[ttl] at (6.2, 2.5) {Hai ngũ giác $2\times C_5$};
\foreach \i in {0,...,4} {
...
\node[label, below=0.05cm] at (6.2, -2.55) {10 nút, mỗi nút bậc 2};

% ====== Equivalence verdict ======
\node[draw, rounded corners, fill=red!8, align=center, font=\scriptsize, text width=11cm]
  (verdict) at (3.1, -3.7)
  {\textbf{1-WL:} cả hai đồ thị $\to$ mọi nút cùng một màu ở \emph{mọi} bước\\
   $\Rightarrow$ \textbf{không MPNN 1-WL nào} phân biệt được $C_{10}$ với $2\times C_5$};
