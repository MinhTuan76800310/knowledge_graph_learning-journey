...
% === Title ===
\node[font=\scriptsize\bfseries] at (3.2, 4.4) {Path explosion $O(\bar{d}^{\,k})$ from a single anchor node};
...
\draw[arrow] (s) -- (a1);
\draw[arrow] (s) -- (a2);
\draw[cut]   (s) -- (a3) node[midway, above, font=\scriptsize, text=red!70!black, sloped] {hub branch};
...
\draw[cut] (a3) -- ++(0.0,-1.0);
\draw[cut] (a3) -- ++(-0.7,-0.9);
\node[label, text=red!70!black] at (6.4, 0.7) {$\deg(\texttt{Time})$ in the\\thousands $\Rightarrow\times$ millions};
...
\node[box, fill=green!8] (prune) at (9.6, 3.0)
  {\textbf{1. Symbolic pruning} (before traversal)\\
   Ontology (Ch4) + SHACL (Ch5) drop mistyped edges\\
   $\Rightarrow \bar{d}\to\bar{d}'\ll\bar{d}$};

\node[box, fill=blue!8] (ppr) at (9.6, 1.3)
  {\textbf{2. PPR prioritization} (instead of enumeration)\\
   $\mathbf{p}=(1{-}\alpha)\tilde{A}\mathbf{p}+\alpha\mathbf{s}$\\
   self-penalizes hubs, lifts the local subgraph};

\node[box, fill=gray!8] (steiner) at (9.6, -0.4)
  {\textbf{3. Steiner $\le 2\times$} (connect terminals)\\
   metric closure + MST, NP-complete (Karp)\\
   over the filtered edge set};

\draw[cut] (a3) -- (prune.west);

% === Error cascade footnote ===
\node[box, fill=red!6, text width=15.5cm] (err) at (4.6, -2.1)
  {\textbf{Multi-hop error cascading}: each hop has precision $p$ $\Rightarrow$ $P(\text{path})=p^{k}$.\\
   With $p{=}0.8$: $k{=}3\Rightarrow0.512$ (a coin flip), $k{=}4\Rightarrow0.41$. Unconstrained traversal both explodes $\bar{d}^k$ and decays $p^k$.};
