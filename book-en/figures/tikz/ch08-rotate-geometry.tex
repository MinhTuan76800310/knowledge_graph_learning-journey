...
\draw[rot] (h) to[bend left=42] node[midway, above, font=\scriptsize, text=orange!70!black] {rotate by $\theta_r$} (t);
...
\node[draw, rounded corners, fill=yellow!10, align=left, font=\scriptsize, text width=6.4cm]
  (legend) at (6.4, 0.2)
  {\textbf{Four relational patterns — one rotation mechanism:}\\[2pt]
   \textcolor{black!70!green}{\textbf{Symmetry}}: $\theta_r \in \{0,\pi\}$ $\Rightarrow r_i=\pm1$\\
   \textcolor{black!70!red}{\textbf{Antisymmetry}}: $\theta_r \notin \{0,\pi\}$\\
   \textbf{Inversion}: $\mathbf{r}_2=\bar{\mathbf{r}}_1$, $\theta_{r_2}=-\theta_{r_1}$\\
   \textbf{Composition}: $\mathbf{r}_3=\mathbf{r}_1\circ\mathbf{r}_2$, $\theta_{r_3}=\theta_{r_1}+\theta_{r_2}$};
...
\begin{scope}[shift={(6.4,-3.4)}]
  ...
  \node[font=\tiny, text=gray!60!black] at (0,-1.35) {reverse rotation};
\end{scope}
