...
\node[font=\footnotesize\bfseries] at (0, 3.9) {Three retrieval families (BOOK-DEFINED)};
...
\node[font=\tiny\itshape, text=gray!60!black, align=center, text width=3.4cm] at (-4.9, -0.5)
  {independent passages,\\no connecting structure};

% === Graph-only ===
\node[box, fill=green!8] (g1) at (0, 3.0) {Graph (structure)};
...
\node[font=\tiny\itshape, text=gray!60!black, align=center, text width=3.4cm] at (5.0, 0.0)
  {the graph drives fetching\\source passages tied to nodes/roles};

\node[font=\scriptsize\itshape, align=center, text width=12.5cm] at (0, -1.2)
  {Each family is strong in a different way — none is absolutely "more correct";\\the router (§9.71) chooses by intent.};
