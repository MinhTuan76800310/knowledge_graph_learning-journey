...
% === Input ===
\node[box, fill=blue!6, minimum width=5cm] (in) at (0, 5.2) {Question + Interpretation (intent, entities, decomposition)};
...
% === Paths (y = 2.3) ===
\node[pathbox, fill=green!10] (p1) at (-5.0, 2.3) {KGQA: SPARQL / graph};
\node[pathbox, fill=green!10] (p2) at (-1.7, 2.3) {Symbolic reasoning};
...
\node[pathbox, fill=orange!12] (p5) at (0, 1.2) {Ledger retrieval (history / contradiction)};
...
% === Answer artifact ===
\node[box, fill=gray!10, minimum width=3.8cm, minimum height=0.7cm] (aa) at (0, -2.7) {Answer artifact (full provenance)};

\draw[arrow] (syn) -- (aa);

% === Principle ===
\node[label, align=center, text width=11cm] at (0, -4.0)
  {Principle: the most precise path that suffices to answer wins;\\every path closes into an Evidence Packet before generation.};
